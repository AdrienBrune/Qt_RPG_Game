    ...
    mSelected(false)
{
    ...
    mHover = false;
    item->setZValue(0);
    qDebug() << "QuickItem creation - " << item->getName();
    ...
    mButtonUse(nullptr)
{
    ...
    if(consumable || tool)
    {
        ...
        mButtonUse->setStyleSheet("QPushButton{padding:2px;border-image:url(:/graphicItems/background_itemInfo);color:white;border:1px solid #404040;}"
...
void W_ItemDisplayer::paintEvent(QPaintEvent *)
{
    ...
    if(mItem != nullptr){
        ...
        QFont font("Arial Narrow",14);
        font.setBold(true);
        painter.setFont(font);

        // DRAW First feature
        QRect fieldImg_1(width()*2/3+5, margin, width()/6-5, width()/6-5);
        QRect fieldTxt_1(fieldImg_1.x()+fieldImg_1.width()+5, fieldImg_1.y()+2, fieldImg_1.width(), fieldImg_1.height());
