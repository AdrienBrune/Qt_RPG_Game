...
void TreeFallen::initMapItem()
{
    ...
    mImageSelected = qrand()%RES_TREE_FALLEN;
    setZValue(Z_TREE_FALLEN);

    mBoundingRect = QRect(0,0,200,100);

    setRotation(qrand()%32-16);
}
...
void Ground::initMapItem()
{
    ...
    mMapItemName = "Ground";
    mImageSelected = 0;
    mImage = QPixmap(":/grounds/grounds/ground_" + buff.asprintf("%d", qrand()%RES_GROUND) + ".png");
    setZValue(Z_GROUND);
...
void Plank::initMapItem()
{
    ...
    mImageSelected = qrand()%RES_PLANK;
    setZValue(Z_PLANK);
...
void Stone::initMapItem()
{
    ...
    mImageSelected = qrand()%RES_STONE;
    setZValue(Z_STONE);
